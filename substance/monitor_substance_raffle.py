...
class SubstanceRaffleMonitor:
    ...
    async def check_raffle(self):
        if self.init_aid is None:
            ...
            if init_aid < 0:
                ...
                while max_aid > min_aid:
                    print(f'初始化实物抽奖id中 {min_aid}, {max_aid}')
                    mid_aid = int((min_aid + max_aid + 1) / 2)
                    is_midaid_exist = await notifier.exec_func(SubstanceRaffleUtilsTask.check, mid_aid)
                    await asyncio.sleep(0.25)

                    print(is_midaid_exist, mid_aid)
                    if not is_midaid_exist:
                        for aid in range(mid_aid + 1, mid_aid + 5):
                            is_aid_exist = await notifier.exec_func(SubstanceRaffleUtilsTask.check, aid)
                            await asyncio.sleep(0.25)
                            if is_aid_exist:
                                min_aid = aid
                                break
                        ...
                        print(f'存在  {mid_aid}')
                        min_aid = mid_aid
                print(f'最新实物抽奖id为 {min_aid}, {max_aid}')
        ...
        while True:
            if self._waiting_pause is not None:
                print(f'暂停启动实物抽奖查找刷新循环，等待RESUME指令')
                await self._waiting_pause
            # 刷新真正的顶点，查看是否b站更新了上限
            while True:
                is_aid_exist = await notifier.exec_func(SubstanceRaffleUtilsTask.check, top_aid)
                await asyncio.sleep(1)
                if not is_aid_exist:
                    print(f'实物抽奖可能不存在或者到达顶点（开区间）{top_aid}')
                    for tmp_aid in range(top_aid + 1, top_aid + 6):
                        is_tmpaid_exist = await notifier.exec_func(
                            SubstanceRaffleUtilsTask.check, tmp_aid)
                        await asyncio.sleep(1)
                        if is_tmpaid_exist:
                            top_aid = tmp_aid
                            break
                    else:
                        print(f'真顶点（开区间） {top_aid}')
                        break  # 只有这里才会真正break，确定真正的顶点（开区间表示本身所在的点不存在抽奖）
                top_aid += 1
            ...
            for curr_aid in range(top_aid-10, top_aid):
                ...
                code, raffles = await notifier.exec_func(
                    SubstanceRaffleUtilsTask.check_and_fetch_raffle, curr_aid)
                cache_aid_code[curr_aid] = code
                print(f'实物抽奖当前CACHE：{cache_aid_code}')
                if not code:
                    print('实物抽奖刷新获取到抽奖信息', *raffles)
    ...
    async def check_result(self):
        while True:
            if self._waiting_pause is not None:
                print(f'暂停启动实物抽奖查找过期循环，等待RESUME指令')
                await self._waiting_pause
    ...
    async def check_handle(self):
        while True:
            ...
            for substance_raffle_status in results:
                ...
                aid = substance_raffle_status.aid
                number = substance_raffle_status.number
                is_exist = await notifier.exec_func(SubstanceRaffleUtilsTask.check, aid)
                if not is_exist:
                ...
                await notifier.exec_task(SubstanceRaffleJoinTask, substance_raffle_status)
    ...
    async def run(self):
        ...
        for substance_raffle_status in results:
            ...
            aid = substance_raffle_status.aid
            number = substance_raffle_status.number
            print(f'正在暴力处理上次中断的{aid}({number})的实物抽奖后续')
            substance_raffle_sql.set_rafflestatus_handle_status(1, aid, number)
