...
class LoginTask(Forced, Wait, Multi):
    ...
    async def login(user):
        ...
        if not json_rsp['code'] and json_rsp['data']:
            token_info = json_rsp['data']['token_info']
            access_key = token_info['access_token']
            refresh_token = token_info['refresh_token']

            rsp = await LoginReq.access_token_2_cookies(user, access_key)
            # type(rsp.cookies) == http.cookies.SimpleCookie
            cookie = ';'.join([f'{morsel.key}={morsel.value}' for morsel in rsp.cookies.values()])

            login_data = {
                'csrf': rsp.cookies['bili_jct'].value,
                'access_key': access_key,
                'refresh_token': refresh_token,
                'cookie': cookie,
                'uid': rsp.cookies['DedeUserID'].value
                }
